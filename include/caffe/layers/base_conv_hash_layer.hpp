...
class BaseConvHashLayer : public Layer<Dtype> {
 ...
 protected:
  ...
  void backward_gpu_gemm(const float *out_col_buf,
	  int bottom_channels, int top_channels, int defined_voxel_num, float *col_buf);
  void weight_gpu_gemm(const float *col_buf, const float* out_col_buf, float *weight_dif,
	  int bottom_channels, int top_channels, int defined_voxel_num);
  void backward_gpu_bias(float* bias, const float* out_col_buf, int defined_voxel_num);
#endif
...
int top_hash2col_gpu(const float *hash_data, const PACKED_POSITION *pos_tags, const int* valid_positions,
	int m_bar, int out_channels, int defined_num, float* col_buff);
